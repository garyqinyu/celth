...
PegScreen::PegScreen(const PegRect & Rect)
{
    ...
    for (uIndex = SYSTEM_POINTER_TYPES;
         uIndex < NUM_POINTER_TYPES;
         uIndex++)
    {
        mpPointers[uIndex].Bitmap = NULL;
    }
...
/*--------------------------------------------------------------------------*/
PegBitmap *PegScreen::CreateBitmap(SIGNED wWidth, SIGNED wHeight, BOOL bTrans)
{
   PegBitmap *pMap = NULL;
...
/*--------------------------------------------------------------------------*/
UCHAR PegScreen::AddPointerType(PegBitmap *pMap, WORD wXOffset, WORD wYOffset)
{
    UCHAR uIndex;
    ...
    for (uIndex = SYSTEM_POINTER_TYPES;
         ...
         uIndex++)\
    {
        if (mpPointers[uIndex].Bitmap == NULL)
        {
            mpPointers[uIndex].Bitmap = pMap;
            mpPointers[uIndex].xOffset = wXOffset;
            mpPointers[uIndex].yOffset = wYOffset;
            return uIndex;
        }
    }
    return 0;
...
/*--------------------------------------------------------------------------*/
void PegScreen::ShowPointer(BOOL bShow)
{
    if (bShow)
    {
        if (!mbShowPointer)
...
{
    if (!mbVirtualDraw && !(Caller->StatusIs(PSF_VISIBLE)))
    {
        return;
    }
    ...
    PegRect PointRect;
    PointRect.Set(x, y, x, y);
    if (!ClipRect(PointRect, Caller->mClip))
    {
        return;
    }
    if (mbVirtualDraw)
    ...
    SIGNED xEnd, SIGNED yEnd, const PegColor &LineColor, SIGNED iWidth)
{
    ...
    if (yStart == yEnd)         // wide horz line?
    {
        ...
        Rectangle(Caller, (const PegRect &) LineRect,
        	(const PegColor &) Color, 0);
...
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
SIGNED PegScreen::TextWidth(const PEGCHAR *Text, PegFont *Font, SIGNED iLen)
{
    SIGNED iWidth = 0;
...
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
SIGNED PegScreen::TextHeight(const PEGCHAR *Text, PegFont *Font)
{
    if (!Text || !Font)
    {
        return 0;
...
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
void PegScreen::DrawText(PegThing *Caller, PegPoint Where, const PEGCHAR *Text,
    ...
    const PegColor &Color, SIGNED iWidth)
{
    ...
    PegPoint *pCurPoint;
    PegPoint CurPoint;
    PegPoint *pUsePoints = pPoints;
    PegRect Bound;

    Bound.Set(mwHRes, mwVRes, 0, 0);

    if (*pPoints != *(pPoints + iNumPoints - 1))
    {
        // Polygon is not closed, close it:
        iNumPoints++;
        pUsePoints = new PegPoint[iNumPoints];
        memcpy(pUsePoints, pPoints, (iNumPoints - 1) * sizeof(PegPoint));
        *(pUsePoints + iNumPoints - 1) = *pUsePoints;
    }

    pCurPoint = pUsePoints;

    for (wLoop = 0; wLoop < iNumPoints; wLoop++)
    {
        CurPoint = *pCurPoint++;

        if (CurPoint.x < Bound.wLeft)
        {
            Bound.wLeft = CurPoint.x;
        }
        if (CurPoint.x > Bound.wRight)
        {
            Bound.wRight = CurPoint.x;
        }

        if (CurPoint.y < Bound.wTop)
        {
            Bound.wTop = CurPoint.y;
        }
        if (CurPoint.y > Bound.wBottom)
        {
            Bound.wBottom = CurPoint.y;
        }
    }

    if (Color.uFlags & CF_FILL)
    {
        FillPolygon(Caller, Bound, pUsePoints, iNumPoints, Color.uBackground);

        if (Color.uForeground == Color.uBackground || !iWidth)
        {
            if (pUsePoints != pPoints)
            {
                delete pUsePoints;
            }
            return;
    ...
    PegPoint *pGet = pUsePoints;

    for (wLoop = 0; wLoop < iNumPoints - 1; wLoop++)
    {
...
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
void PegScreen::FillPolygon(PegThing *Caller, PegRect &Bound, PegPoint *pPoints,
    SIGNED iNumPoints, COLORVAL Color)
{
    if (!mbVirtualDraw && !(Caller->StatusIs(PSF_VISIBLE)))
    {
        return;
    ...
    if (!ClipRect(Bound, Caller->mClip))
    {
        return;
    }
    ...
    if (mbVirtualDraw)
    {
        FillPolygonView(Bound, pPoints, iNumPoints, Color);
        return;
    }
    ...
    while(pView)
    {
        ...
        {
            if (pView->mView.Overlap(Bound))
            {
                PegRect tRect = Bound & pView->mView;
                FillPolygonView(tRect, pPoints, iNumPoints, Color);
            }
        }
        pView = pView->pNext;
    }
    ...
    FillPolygon(Bound, pPoints, iNumPoints, Color);

#endif
}

/*--------------------------------------------------------------------------*/
// Edge Class Definition- Used for complex polygon fill
/*--------------------------------------------------------------------------*/
class Edge
{
    public:
        Edge(int x1, int y1, int dx, int dy);
        Edge *NextEdge;
        int x;
        int y;
        int move_x;
...
/*--------------------------------------------------------------------------*/
// Edge Class Implementation- Used for complex polygon fill
/*--------------------------------------------------------------------------*/
Edge::Edge(int x1, int y1, int dx, int dy)
{
    int width;
...
// FillPolygonView:
//
// This is a scan-line oriented fill algorithm. First, we outline the
// polygon in memory. Then we start scanning left to right. As we scan,
// we keep track of whether or not we are inside the polygon, and draw
// horizontal lines to fill the polygon area.
//
/*--------------------------------------------------------------------------*/
void PegScreen::FillPolygonView(const PegRect &View, PegPoint *pPoints,
     SIGNED iNumPoints, COLORVAL Color)
{
	Edge *global_edge_tablePtr, *active_edge_tablePtr;
	Edge *active_edge_tableEdge, **active_edge_tableEdgePtr, *TempEdge;
	Edge *PresentEdge;
	Edge **PresentEdgePtr;
	Edge *NewEdgePtr;
	Edge *NextEdge, **NextEdgeLink;

	int i, xs, ys, xe, ye, dy, dx, temp, x, y,
		LeftX, swapped;

	global_edge_tablePtr = NULL;

    xs = (pPoints + iNumPoints - 1)->x;
    ys = (pPoints + iNumPoints - 1)->y;
	...
	for (i = 0; i < iNumPoints; i++)
	{
		...
		if ((dy = ye - ys) != 0)
		{
			...
			if ((NewEdgePtr = new Edge(xs,ys,dx,dy))==NULL)
            {
				return;
            }
			NextEdgeLink = &global_edge_tablePtr;

			while (((NextEdge = *NextEdgeLink) != NULL) &&
				(NextEdge->y <= ys) && ((NextEdge->y != ys) ||
				(NextEdge->x < xs)))
            {
				NextEdgeLink = &NextEdge->NextEdge;
            }
			NewEdgePtr->NextEdge = NextEdge;
			*NextEdgeLink = NewEdgePtr;
		}
		xs = (pPoints + i)->x;
		ys = (pPoints + i)->y;
	}

	active_edge_tablePtr = NULL;
	y = global_edge_tablePtr->y;

	while ((global_edge_tablePtr != NULL) || (active_edge_tablePtr != NULL))
	{
		active_edge_tableEdgePtr = &active_edge_tablePtr;

		while ((global_edge_tablePtr != NULL) && (global_edge_tablePtr->y == y))
		{
			x = global_edge_tablePtr->x;

			for (;;)
			{
				active_edge_tableEdge = *active_edge_tableEdgePtr;
				if ((active_edge_tableEdge == NULL) || (active_edge_tableEdge->x >= x))
				{
					TempEdge = global_edge_tablePtr->NextEdge;
					*active_edge_tableEdgePtr = global_edge_tablePtr;
					global_edge_tablePtr->NextEdge = active_edge_tableEdge;
					active_edge_tableEdgePtr = &global_edge_tablePtr->NextEdge;
					global_edge_tablePtr = TempEdge;
					break;
				}
				else
                {
					active_edge_tableEdgePtr = &active_edge_tableEdge->NextEdge;
                }
			}
		}
		PresentEdge = active_edge_tablePtr;

        if (y >= View.wTop && y <= View.wBottom)
        {
		    while (PresentEdge != NULL)
		    {
			    LeftX = PresentEdge->x;
			    PresentEdge = PresentEdge->NextEdge;
                ...
                HorizontalLine(LeftX, xe, y, Color, 1);
			    PresentEdge = PresentEdge->NextEdge;
		    }
        }
		PresentEdgePtr = &active_edge_tablePtr;

		while ((PresentEdge = *PresentEdgePtr) != NULL)
		{
			if ((--(PresentEdge->Count)) == 0)
			{
				*PresentEdgePtr = PresentEdge->NextEdge;
				delete(PresentEdge);
			}
			else
			{
				PresentEdge->x += PresentEdge->move_x;
				if ((PresentEdge->decision +=
					PresentEdge->decisionUp) > 0)
					{
						PresentEdge->x += PresentEdge->xdir;
						PresentEdge->decision -=
							PresentEdge->decisionDown;
					}
					PresentEdgePtr = &PresentEdge->NextEdge;
			}
		}
		if (active_edge_tablePtr != NULL)
		{
			do
			{
				swapped = 0;
				PresentEdgePtr = &active_edge_tablePtr;
				while ((PresentEdge = *PresentEdgePtr)->NextEdge != NULL)
				{
					if (PresentEdge->x > PresentEdge->NextEdge->x)
					{
						TempEdge = PresentEdge->NextEdge->NextEdge;
						*PresentEdgePtr = PresentEdge->NextEdge;
						PresentEdge->NextEdge->NextEdge = PresentEdge;
						PresentEdge->NextEdge = TempEdge;
						swapped = 1;
					}
					PresentEdgePtr = &(*PresentEdgePtr)->NextEdge;
				}
			} while (swapped != 0);
		}
		y++;
    ...
    DWORD dPattern)
{
    ...
    if (mbVirtualDraw)
    {
        PatternLine(wXStart, wYStart, wXEnd, wYEnd, Rect,
        (const PegColor &) Color, 
        wWidth, dPattern);
        return;
    }
    ...
    while(pView)
    {
        ...
        {
            PatternLine(wXStart, wYStart, wXEnd, wYEnd, Rect,
            	(const PegColor &) Color, wWidth, dPattern);
            break;
        }
        else
        {
            ...
            {
                tRect = Rect & pView->mView;
                PatternLine(wXStart, wYStart, wXEnd, wYEnd, tRect,
                	(const PegColor &) Color, wWidth, dPattern);
            }
        }
        pView = pView->pNext;
    ...
    PegColor Color, SIGNED iWidth)
{
    ...
    if (mbVirtualDraw)
    {
        if (mVirtualRect.Contains(Bound))
        {
            EllipseFast(Bound, Color, iWidth);
        }
        else
        {
            EllipseToView(Bound, Color, iWidth, mVirtualRect);
        }
        return;
    }
...
void PegScreen::GenerateViewportList(PegThing *pStart)
{
    ...
    if (pStart == pStart->Presentation() ||
        pStart->Parent() == pStart->Presentation())
    {
        AddViewport(pStart, pStart->mReal);
        return;
    }

    // If I am not starting at the level of a top-level window,
    // I need to see if any other top-level windows are covering
    // my top level window.

    pNewStart = pStart;
    PegThing *pTop = pStart->Parent();
    while(pTop->Parent() != pTop->Presentation())
    {
        if (pTop->Parent()->First() != pTop)
        {
            pNewStart = pTop->Parent();
        }
        pTop = pTop->Parent();      // get pointer to top-level window
    }

    if (pNewStart != pStart)
    {
        GenerateViewportList(pNewStart);
        return;
    }

    // if we get to here, no other windows are above this window,
    // just do a viewport Add:
...
PegBitmap *PegScreen::ResizeImage(PegBitmap *pSrc, SIGNED iWidth, SIGNED iHeight)
{
    ...
    RectStretch(pSrc->wWidth - 1, pSrc->wHeight - 1,
        iWidth - 1, iHeight - 1, pSrc, pTarget);
...
/*--------------------------------------------------------------------------*/
void PegScreen::RectStretch(SIGNED SrcWidth, SIGNED SrcHeight,
    SIGNED DestWidth, SIGNED DestHeight, PegBitmap *pSrc, PegBitmap *pDest)
{
	...
	for(d = 0; d <= dx; d++)
	{
		Stretch(DestWidth, SrcWidth, yCurrent, yEnd, pSrc, pDest);
		while(e >= 0 && dx2)
		{
            yCurrent++;
			e -= dx2;
...
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
void PegScreen::Stretch(SIGNED DestWidth, SIGNED SourceWidth,
    SIGNED yCurrent, SIGNED yEnd, PegBitmap *pSrc, PegBitmap *pDest)
{
	SIGNED dx,dy,e,d,dx2;
    SIGNED xCurrent = 0;
    SIGNED xEnd = 0;
	...
	for (d = 0; d <= dx; d++)
	{
		color = GetBitmapPixel(xCurrent, yCurrent, pSrc); 
		PutBitmapPixel(xEnd, yEnd, pDest, color);
...
#define AA_COLORS 15
//
// Minimal implementation of Anti-Aliased line drawing. Assumes we have
// a linear range of colors between ForeColor and ForeColor + AA_COLORS.
// This of course is only true for the 8-bit or higher screen drivers, and
// then only with proper palette configuration. This routine must be improved
// to support grayscale, 2-bpp, and monochrome (dithered) anti-aliased line
// drawing in future releases.
/*--------------------------------------------------------------------------*/
void PegScreen::AALine(SIGNED wXStart, SIGNED wYStart, SIGNED wXEnd, 
    SIGNED wYEnd, COLORVAL ForeColor, SIGNED wWidth)
{
    if (wYStart == wYEnd)
    {
        HorizontalLine(wXStart, wXEnd, wYStart, ForeColor, wWidth);
        return;
    }
    if (wXStart == wXEnd)
    {
        VerticalLine(wYStart, wYEnd, wXStart, ForeColor, wWidth);
        return;
    }
    ...
    if (dx >= dy)
    {
             ...
             wCurx <= wNextx; wCurx++, wNextx--, decision += dy)
        {
            ...
            DrawColor = ((decision << 4) / dx) * y_sign;

            wpy = wCury - wWidth / 2;
            PlotPointView(wCurx, wpy, ForeColor + TopOffset + DrawColor);
            wpy++;

            for (;wpy < wCury + wWidth / 2; wpy++)
            {
                PlotPointView(wCurx, wpy, ForeColor);
            }

            PlotPointView(wCurx, wpy, ForeColor + BottomOffset - DrawColor);

            wpy = wNexty - wWidth / 2;
            PlotPointView(wNextx, wpy, ForeColor + BottomOffset - DrawColor);
            wpy++;

            for (;wpy < wNexty + wWidth / 2; wpy++)
            {
                PlotPointView(wNextx, wpy, ForeColor);
            }
            PlotPointView(wNextx, wpy, ForeColor + TopOffset + DrawColor);
        }
    }
    else
    {
            ...
            wCury <= wNexty; wCury++, wNexty--, decision += dx)
        {
            ...
            DrawColor = ((decision << 4) / dy) * x_sign;

            wpx = wCurx - wWidth / 2;
            PlotPointView(wpx, wCury, ForeColor + TopOffset + DrawColor);
            wpx++;

            for (;wpx < wCurx + wWidth / 2; wpx++)
            {
                PlotPointView(wpx, wCury, ForeColor);
            }
            PlotPointView(wpx, wCury, ForeColor + BottomOffset - DrawColor);

            wpx = wNextx - wWidth / 2;
            PlotPointView(wpx, wNexty, ForeColor + BottomOffset - DrawColor);
            wpx++;

            for (;wpx < wNextx + wWidth / 2; wpx++)
            {
                PlotPointView(wpx, wNexty, ForeColor);
            }
            PlotPointView(wpx, wNexty, ForeColor + TopOffset + DrawColor);
        }
    }
}
