...
void PegWindow::InitClient(void)
{
    ...
    if (mpVScroll)
    {
        ...
        if (StatusIs(PSF_VISIBLE))
        {
            mpVScroll->Message(PegMessage(PM_PARENTSIZED));
        }
    }
    if (mpHScroll)
    {
        ...
        if (StatusIs(PSF_VISIBLE))
        {
            mpHScroll->Message(PegMessage(PM_PARENTSIZED));
        }
    }
}
...
{
    PegRect FillRect;
    FillRect.wLeft = mpHScroll->mReal.wRight + 1;
    FillRect.wTop = mpVScroll->mReal.wBottom + 1;
    FillRect.wRight = mpVScroll->mReal.wRight;
    FillRect.wBottom = mpHScroll->mReal.wBottom;
    PegColor Color(PCLR_BUTTON_FACE, PCLR_BUTTON_FACE, CF_FILL);
