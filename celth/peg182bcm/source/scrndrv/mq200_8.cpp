/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
// mq200_8.cpp - PegScreen screen driver for MediaQ 200 video
...
// be LCD, CRT or a combination. If you are using other than 256 colors,
...
#include "peg.hpp"
#include "mqregdef.hpp" // MediaQ Register Definitions

#if defined(USE_PCI)
#include "pegpci.hpp"
#endif

#ifdef GRAYSCALE        // running in 16-grayscale mode?
...
//#define BLIT_CHECK    WaitEngineFIFO(16);  // make sure engine is free
#define BLIT_CHECK      // doesn't seem to cause a problem to access vid mem
    ...
    COLORVAL Color, SIGNED wWidth)
{
    ...
    while(wWidth-- > 0)     // this version draws by hand, no "snow"
    {
        Put = mpScanPointers[wYPos] + wXStart;
...
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
void MQ200Screen::SetPalette(SIGNED iFirst, SIGNED iNum, const UCHAR *Get)
{
	SIGNED i;
    ...
    UCHAR *pReg = mpVidRegBase + C1_BASE;
    DWORD *pPut = (DWORD *) pReg;
    pReg = DefPalette256;
    DWORD dVal;
...
//	if (ulClassRevID != MQ200_REV_1A)
    ...
    *(pReg + HW_CURSOR1_BGCLR) = 0xffffffL;
    ...
    pReg = (DWORD *) (mpVidRegBase + GE_BASE);
    *(pReg + DEST_STRIDE) = mwHRes;     // 8BPP == 0 in hi bits
