/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
// ct690008.cpp - Linear 256 color screen driver for C&T69000 on linear
// bus, such as PCI bus.
//
//
...
//
// MODE320:  Configure for 320x240  full color CRT and/or LCD.
...
//
// All available configuration flags are found in the ct690008.hpp header file.
//
//
// Known Limitations:
// --------------------------------------------------------------------------------------------
    ...
    COLORVAL Color, SIGNED wWidth)
{
    ...
    while(wWidth-- > 0)
    {
        pPut = mpScanPointers[wYPos] + wXStart;
        memset(pPut, Color, iLen);
        wYPos++;
    }
   #else
    ...
    *(pReg + 3) = 0;
    ...
    if (mbVirtualDraw)
    {
        *pReg = ((DWORD) mVirtualRect.Width() << 16);
        dOffset *= mVirtualRect.Width();
        dOffset += (DWORD) mpScanPointers[0] - (DWORD) mpVidMemBase;
    }
    else
    {
     ...
     const SIGNED xMove, const SIGNED yMove)
{
   ...
   #ifdef DOUBLE_BUFFER
    DWORD dOffset = View.wTop + mwVRes;
   #else
    ...
    *(pReg + 4) = wCopyStart|0xcc;     // SRCCPY ROP
    ...
    if (wCopyStart == CS_LOWER_RIGHT)
    {
        if (mbVirtualDraw)
        {
            //dOffset += (DWORD) iHeight * dPitch;
            dOffset += (DWORD) (iHeight - 1)  * dPitch;
        }
        else
        {
            //dOffset += (DWORD) iHeight * mwHRes;
...
/*--------------------------------------------------------------------------*/
#if defined(MODE320) || defined(MODE640) || defined(MODE800) || defined(MODE1024)
#else
...
#if defined(MODE320)
//{0xc8,  0xd1},       // DotClk2 VCO M-Div Low
{0xc8,  0xe5},       // DotClk2 VCO M-Div Low
{0xc9,  0x1e},       // DotClk2 VCO N-Div Low
{0xca,  0x00},       // DotClk2 VCO M-N-Div High
{0xcb,  0x44},       // DotClk2 Divisor Select
...
{0xc9,  0x14},       // DotClk2 VCO N-Div Low
{0xca,  0x00},       // DotClk2 VCO M-N-Div High
...
{0x17,  0x57},       // Polynomial FRC

{0x18,  0x00},       // Text Mode Ctrl
{0x19,  0x88},       // Blink Rate
{0x1a,  0x00},       // STN Buffering
//{0x1e,  0x80},       // ACDCLK Control
{0x1e,  0x40},       // ACDCLK Control



#if defined(MODE320)
{0x20,  0x27},       // Horz Size LSB
{0x21,  0x29},       // Horz Sync Start LSB
{0x22,  0x0e},       // Horz Sync End
{0x23,  0x2f},       // Horz Total LSB
{0x24,  0x28},       // Hsynch Delay LSB
{0x25,  0x00},       // Horz Overflow 1
{0x26,  0x00},       // Horz Overflow 2
{0x27,  0x0b},       // HSynch Width
{0x30,  0xef},       // Vert Size LSB
{0x31,  0x00},       // Vert Synch LSB
{0x32,  0x0c},       // Vert Synch End
{0x33,  0x07},       // Vert Total LSB
{0x34,  0x00},       // VSynch Delay LSB
{0x35,  0x10},       // Vert Overflow 1
{0x36,  0x01},       // Vert Overflow 2
#elif defined(MODE640)
