/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
// 1356scr.cpp - PegScreen screen driver for Seiko-Epson 1356 video
...
#ifdef PEGWIN32
// Here if running the SDU1356 Eval card under Win32:
SED1356Screen::SED1356Screen(HWND hWnd, PegRect &Rect) : PegScreen(Rect)
#else
...
#endif
{
   ...
   // This section of code is only needed for running the SDU1356 eval
...
PegBitmap *SED1356Screen::CreateBitmap(SIGNED wWidth, SIGNED wHeight, BOOL bHasTrans)
{
    ...
    // A BUG IN THE 1356 BITBLT ENGINE. If the bitmap width is odd,
...
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
void SED1356Screen::SetPalette(SIGNED iFirst, SIGNED iNum, const UCHAR *Get)
{
...
void SED1356Screen::MemoryToScreen(void)
{
    ...
    DWORD dDest = (DWORD) (mpVidMemBase + (mpScanPointers[iTop] - mpScanPointers[0]) + iLeft);	// JMD
    BLIT_CHECK
