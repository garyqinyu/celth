...
void PegIdleFunction(void)
{
	...
	if(CELTH_Symbol_Enquire_Value("peg_process_msgbox",&pMsgQueue))
	{
		printf("no message box for the peg window system!!!\n");
		return ;
	}
	...
	eStatus = CELTH_MsgWait(pMsgQueue,pvMsg, 0xFFFFFFFF); /* wait for message */
