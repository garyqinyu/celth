...
/*--------------------------------------------------------------------------*/
void PegTextBox::DataSet(const PEGCHAR *Text)
{
...
void PegTextBox::DrawAllText(void)
{
    ...
    PutPoint.x = mClient.wLeft + TB_WHITESPACE - miLeftOffset;
    PutPoint.y = mClient.wTop + TB_WHITESPACE;
    ...
    SIGNED iStopy = mClient.wBottom - TB_WHITESPACE;
...
/*--------------------------------------------------------------------------*/
PEGCHAR *PegTextBox::GetLineStart(SIGNED iLine, SIGNED *pPutLen)
{
    ...
    PEGCHAR *pGet = DataGet() + mwLineStarts[iNewStart];
...
{
    PEGCHAR *pStart = GetLineStart(iLine, NULL);
    if (pStart)
    {
...
/*--------------------------------------------------------------------------*/
PEGCHAR *PegTextBox::FindLinePointer(SIGNED iLine)
{
    PEGCHAR *pCurrent = DataGet();
    SIGNED iLoop = 0;
    SIGNED sMax = mClient.Width() - 5;
...
/*--------------------------------------------------------------------------*/
PEGCHAR *PegTextBox::FindNextLine(PEGCHAR *Start, SIGNED sMaxWidth, SIGNED iDir)
{
...
void PegTextBox::GetVScrollInfo(PegScrollInfo *Put)
{
    ...
    if (miTotalLines > Put->wMax + 1)
    {
        Put->wMax = miTotalLines - 1;
    }
}
