...
#define USER_POINTER_TYPES   5
#define NUM_POINTER_TYPES (SYSTEM_POINTER_TYPES + USER_POINTER_TYPES)
...
class PegScreen 
{
    public:
        ...
        virtual void AALine(SIGNED wXStart, SIGNED wYStart, SIGNED wXEnd, 
            SIGNED wYEnd, COLORVAL ForeColor, SIGNED wWidth);

	   #ifdef PLOTPOINTVIEW_AS_FUNCTION
		virtual void PlotPointView(SIGNED x, SIGNED y, COLORVAL c) = 0;
	   #endif
        ...
        virtual void Line(PegThing *Caller, SIGNED wXStart, SIGNED wYStart,
            ...
            SIGNED wWidth = 1);
/*
        virtual void PolyLine(PegThing *Caller, SIGNED *pPoints, WORD wNumPoints,
            const PegColor &Color, SIGNED wWidth = 1) = 0;
*/
        ...
        virtual void SetFillPattern(PegThing *Caller, PegBitmap *Getmap)
        {
        ...
        virtual void DrawText(PegThing *Caller, PegPoint Where,
            const PEGCHAR *Text, PegColor &Color, PegFont *Font, 
            SIGNED Count = -1);

        virtual SIGNED TextHeight(const PEGCHAR *Text, PegFont *Font);

        virtual SIGNED TextWidth(const PEGCHAR *Text, PegFont *Font, SIGNED iLen = -1);
        ...
        UCHAR AddPointerType(PegBitmap *pMap, WORD wXOffset, WORD wYOffset);

        UCHAR GetPointerType(void);

        PegBitmap *GetPointer(void) {return mpCurPointer;}

        virtual void ShowPointer(BOOL bShow);
        ...
        PegRect GetInvalidRect(void) {return mInvalid;}
        ...
        virtual PegFont *MakeFont(UCHAR uHeight, BOOL bBold = FALSE,
            BOOL bItalic = FALSE);
        ...
        virtual DWORD NumColors(PegThing *) {return mdNumColors;}
       #endif


       #ifdef PEG_FULL_GRAPHICS

        virtual void Polygon(PegThing *Caller, PegPoint *pPoints,
            SIGNED iNumPoints, const PegColor &Color, SIGNED iWidth = 1);

        virtual void Circle(PegThing *Caller, SIGNED xCenter, SIGNED yCenter, SIGNED r,
            PegColor &Color, SIGNED iWidth = 0);

        virtual void Ellipse(PegThing *Caller, const PegRect &Bound,
            PegColor Color, SIGNED iWidth);

        virtual void PatternLine(PegThing *Caller, SIGNED wXStart, SIGNED wYStart,
        ...
        virtual PegBitmap *ResizeImage(PegBitmap *pSrc, SIGNED iWidth, SIGNED iHeight);

       #endif

       #ifdef PEG_FP_GRAPHICS

        virtual void Arc(PegThing *Caller, SIGNED xc, SIGNED yc, SIGNED radius,
    ...
    protected:
        ...
        virtual void FillPolygon(PegThing *Caller, PegRect &Bound,
            PegPoint *pPoints, SIGNED iNumPoints, COLORVAL Color);
        ...
        virtual void EllipseToView(const PegRect &Bound, PegColor &color, SIGNED width,
            PegRect rView);

        virtual void EllipseFast(const PegRect &Bound, PegColor &color, SIGNED width);
        ...
        virtual void ArcToView(SIGNED xc, SIGNED yc, SIGNED radius, float start_angle,
	        float end_angle, PegColor Color, SIGNED width, PegRect &View);

        virtual void CheckArcPoint(SIGNED x, SIGNED y, SIGNED test_var,
            SIGNED type, SIGNED start, SIGNED end, PegRect &View);

        virtual void ArcLine(SIGNED xs, SIGNED ys, SIGNED xe, SIGNED ye, PegRect &View);

        virtual void ArcPlot(SIGNED x, SIGNED y, SIGNED test_var, SIGNED type, COLORVAL color,
	        SIGNED start_test, SIGNED end_test, PegRect &View);

        virtual void ArcFill(SIGNED xc, SIGNED yc, SIGNED radius, float start_angle,
	        float end_angle, PegColor color, PegRect &mView);

       #endif

       #ifdef PEG_VECTOR_FONTS
        virtual void FontLine(UCHAR *pData, WORD wBitOffset,
            PegPoint Start, PegPoint End, WORD wBytesPerRow);
       #endif

        virtual void InitVidMemManager(UCHAR *pStart, UCHAR *pEnd);
        ...
        PegBitmap *mpFillBitmap;

#ifdef USE_DWORD_SCANPTR
		DWORD PEGFAR **mpScanPointers;
		DWORD PEGFAR **mpSaveScanPointers;
#else
    ...
    private:
        ...
        void RectStretch(SIGNED xs2,SIGNED ys2,
            SIGNED xd2,SIGNED yd2, PegBitmap *pSrc, PegBitmap *pDest);

        void Stretch(SIGNED x2, SIGNED y2,
