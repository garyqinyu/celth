...
//
//  PEGCHAR   usually signed character
...
/*--------------------------------------------------------------------------*/
// The WORD data type can be 16 or 32 bits, unsigned. If your are using a
// 32-bit CPU, you should probably use 32-bit WORD (remove the "short" below)
// for best performance, although heap usage will probably increase slightly.
/*--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------*/
// The SIGNED datatype can be 16 or 32 bits, signed. We default to 16-bits to
// run well on 16 or 32 bit CPU types. If you are using a 32-bit CPU, you will
// usually gain better performance by defining SIGNED as 32-bits, i.e.
// (remove the keyword "short" below.
/*--------------------------------------------------------------------------*/
...
#else
typedef char PEGCHAR;          // 8 bit (preferably) unsigned, no UNICODE
#endif
...
const WORD TCF_FORCEFIT      =  0x0001;
const WORD TCF_HCENTER       =  0x0100;
const WORD TCF_HLEFT         =  0x0200;
const WORD TCF_HRIGHT        =  0x0400;
const WORD TCF_VCENTER       =  0x0800;
const WORD TCF_VTOP          =  0x1000;
const WORD TCF_VBOTTOM       =  0x2000;
...
const WORD PSF_DEFAULT_FOCUS =     0x0100;
const WORD PSF_CONTINUOUS_SCROLL = 0x0200;
const WORD PSF_TAB_STOP =          0x0400;
const WORD PSF_OWNS_POINTER =      0x0800;
...
const COLORVAL TRANSPARENCY   = (COLORVAL) 0x0001; // transparent color

#if (PEG_NUM_COLORS == 65535)   // 16-bit color mode?
...
const COLORVAL RED             = 2;        
const COLORVAL GREEN           = 4;
const COLORVAL BROWN           = 7;        
const COLORVAL BLUE            = 1;        
const COLORVAL MAGENTA         = 3;
const COLORVAL CYAN            = 5;
const COLORVAL LIGHTGRAY       = 12;  
const COLORVAL DARKGRAY        = 7;   
const COLORVAL LIGHTRED        = 4;   
const COLORVAL LIGHTGREEN      = 9; 
const COLORVAL YELLOW          = 13;
const COLORVAL LIGHTBLUE       = 3;  
const COLORVAL LIGHTMAGENTA    = 6;
const COLORVAL LIGHTCYAN       = 10;  
const COLORVAL WHITE           = 15;

#else

// here for 16 color VGA palette
...
#define   BMF_ROTATED      0x04     // bitmap is rotated 90 or 270 degrees
#define   BMF_GRAYSCALE    0x08     // bitmap uses grayscale palette
...
struct PegBitmap
{
    ...
    UCHAR uBitsPix;        // 1, 2, 4, 8, 16, or 24
...
#define IS_GRAYSCALE(a) (a->uFlags & BMF_GRAYSCALE)
