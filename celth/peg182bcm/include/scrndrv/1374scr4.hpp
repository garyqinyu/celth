/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
// 1374scr4.hpp - Linear screen driver for 4-bpp (16 color) operation.
//
...
/*--------------------------------------------------------------------------*/
// Default LCD resolution. Since there is only 40k of memory on the 
// SED1374, 320x240 is the highest resolution it supports at 4bpp.
/*--------------------------------------------------------------------------*/
...
#define VID_MEM_BASE    0xD0000L    
#define VID_REG_BASE    VID_MEM_BASE + 0xFFE0    

/*--------------------------------------------------------------------------*/
// VID_MEM_SIZE- how much video memory is installed on your system?
/*--------------------------------------------------------------------------*/

#define VID_MEM_SIZE    0xA000     // default to 40k video RAM

/*--------------------------------------------------------------------------*/
// ID of the SED1374 card
/*--------------------------------------------------------------------------*/
#define SED1374ID		0x18
...
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
class SED1374Screen4 : public PegScreen
{
    public:
       #ifndef PEGWIN32
        SED1374Screen4(PegRect &);
       #else
        SED1374Screen4(HWND hwnd, PegRect &);
        SIGNED mwWinRectXOffset;
        SIGNED mwWinRectYOffset;
       #endif

        virtual ~SED1374Screen4();
