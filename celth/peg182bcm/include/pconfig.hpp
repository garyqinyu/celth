...
/*
  ...
  2) ETS/TNT      (contact VentureCom, Inc.)
 ...
 14) Enea OSE     (contact Enea OSE Systems)
 15) OnTime-32    (contact On Time Informatik)
 16) VRTX         (contact Mentor Graphics Incorporated)
 17) Embedix      (contact Lineo, Inc.)
...
/*--------------------------------------------------------------------------*/
// to build the library with no RTOS, for example for DOS, turn on PEGSTANDALONE. 
/*--------------------------------------------------------------------------*/

//#define PEGSTANDALONE

/*--------------------------------------------------------------------------*/
// to build for Win32, turn off the above define, and include this:
/*--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------*/
// to build for SMX turn off the above targets and  
...
/*--------------------------------------------------------------------------*/
// to build the library for On Time RTOS-32, turn on PEGONTIME
/*--------------------------------------------------------------------------*/

//#define PEGONTIME          // turn this on to run under On Time RTOS-32
...
/*--------------------------------------------------------------------------*/
// to build for uC/OS turn off all of the above and
// uncomment the line below:
/*--------------------------------------------------------------------------*/
...
//#define SOLARISPEG    // turn this on to run with Solaris x86
...
//#define X11SOLARIS    // running on top of Solaris
...
//#define PSCREEN  "l24scrn.hpp"     // substitute this if using l24scrn.cpp
...
//#define PSCREEN  "lh531_4.hpp"     // Sharp LH79531- 4 bpp
//#define PSCREEN  "lh531_8.hpp"     // Sharp LH79531- 8 bpp
//#define PSCREEN  "7111gray.hpp"    // gray-scale for Cirrus Logic 7111 ARM
//#define PSCREEN  "7111mono.hpp"    // monochrome for Cirrus Logic 7111 ARM
//#define PSCREEN  "7212gray.hpp"    // gray-scale for Cirrus Logic 7212 ARM
//#define PSCREEN  "7212mono.hpp"    // monochrome for Cirrus Logic 7212 ARM
//#define PSCREEN  "7312scr8.hpp"    // 8bpp for Cirrus Logic 7312 ARM
...
//#define PSCREEN  "13a0scr8.hpp"    // S1D13A04 eval card, 8-bpp all res
...
//#define PSCREEN  "ct69_24.hpp"     // C&T 69000 24bpp color 
...
//#define PSCREEN  "smscreen.hpp"    // Silicon Motion video card
//#define PSCREEN  "flpscrn1.hpp"	 // substitute this if using flpscrn1.cpp
//#define PSCREEN  "a400_16.hpp"     // Sharp 7A400- 16-bpp
#define PSCREEN  "omapscrn.hpp"    // TI OMAP platform with Sharp TFT display
#endif
...
/*--------------------------------------------------------------------------*/
// The following flag is used only when running at 16 color (4-bpp) output,
// which may be either color (VGA), or more commonly grayscale. This
// definition determines which set of color definitions are used.
/*--------------------------------------------------------------------------*/

#if (PEG_NUM_COLORS == 16)

#define GRAYSCALE               // Default is ON, change to off
                                // if you are using VGA mode.
#endif
...
/*--------------------------------------------------------------------------*/
// If the library must support different screen drivers without being
// recompiled, define PLOTPOINTVIEW_AS_FUNCTION
/*--------------------------------------------------------------------------*/

//#define PLOTPOINTVIEW_AS_FUNCTION
...
/*--------------------------------------------------------------------------*/
// The following definitions fine-tune how PEG handles navigation when the
// input model includes handling keyboard messages. This may mean that you
// have a true keyboard, or it may mean that you have another type of input
// device and the most logical method of navigation is to send PEG 
// TAB or arrow key messages for moving from selection to selection.
//
// These definitions are not exclusive, i.e. you can turn them both on if
// your interface supports both up/down/right/left and "next/previous"
// navigation styles.
/*--------------------------------------------------------------------------*/
#define PEG_ARROW_KEY_SUPPORT  // navigate with up/down/left/right arrow keys?
#define PEG_TAB_KEY_SUPPORT    // navigate with next/previous (tab) keys?


#endif

/*--------------------------------------------------------------------------*/
// The following definition determines how PEG operates if last visible object
// is removed. When running under Win32, we want PEG to exit so that the
// Windows appliation terminates. When running on your target, you usually
// never want the PEG task to terminate, even if all windows are closed.
/*--------------------------------------------------------------------------*/

#ifdef PEGWIN32
#define EXIT_PEG_WHEN_LAST_CLOSED       // default for Win32 environment
#else
//#define EXIT_PEG_WHEN_LAST_CLOSED     // turned off for embedded target
#endif

/*--------------------------------------------------------------------------*/
// Printer driver (optional).  As with the screen driver, the printer driver
// header file must correspond with the printer driver source file you compile
// and link into your system.
/*--------------------------------------------------------------------------*/

//#define PEG_PRINTER_SUPPORT

#ifdef PEG_PRINTER_SUPPORT
#define PEG_PRINT "hpprnt.hpp"      // HP PCL3 compatible printer driver
#endif
...
*/
/*--------------------------------------------------------------------------*/
// The following define will include a PegDirectoryBrowser to the
// library. This option is only supported on Unix style OS's.
/*--------------------------------------------------------------------------*/
/*
#if defined(POSIXPEG) || defined(PEGX11)
...
/*--------------------------------------------------------------------------*/
//
// The following define, PEG_RUNTIME_TRUETYPE_SUPPORT, pulls in the ported
// FreeType TrueType font renderer. This provides the ability to render
// a TrueType font from a file or from memory, at runtime, to a PegFont of
// any pixel size.
//
// This should NOT be turned on unless you are certain that you need it
// because it requires ~68 KBytes (x86 protected mode) of extra code.
//
/*--------------------------------------------------------------------------*/

//#define PEG_RUNTIME_TRUETYPE_SUPPORT
...
/*--------------------------------------------------------------------------*/
#if defined(PHARLAP) || defined(PEGX) || defined(PEGSMX) || defined(PEGONTIME)
...
#ifdef OT_UNICODE             // Building for OnTime Unicode?
#define PEG_UNICODE           // use 16-bit character encoding
#endif


/*--------------------------------------------------------------------------*/
// The following definition, PEGWIDECHAR, should be turned on only if your
// compiler supports the wchar_t (sometimes wchar, etc...) two-byte character
// data type AND YOU WANT TO USE IT. If your compiler doesn't support wchar,
// or if your compiler supports wchar but you don't want to use it, leave
// this turned off.
/*--------------------------------------------------------------------------*/

//#define PEGWIDECHAR wchar_t
...
/*--------------------------------------------------------------------------*/
// PEG_BIG5_CONVERSION- Include table and functions for converting between
// Chinese BIG5 encoding and Unicode character encoding?
/*--------------------------------------------------------------------------*/

#if defined(PEG_UNICODE)
#define PEG_BIG5_CONVERSION // include Chinese BIG5 to Unicode functions
#endif
...
// of the common string functions. If you are building for UNICODE, 
...
#ifdef PEG_UNICODE
#define PEG_STRLIB          // default to PEG string functions if UNICODE
#else
...
#ifdef PEG_UNICODE
#define USE_PEG_LTOA        // default to PEG version if UNICODE
#else
...
/*--------------------------------------------------------------------------*/
/*
$Workfile: pconfig.hpp $
$Date: 10/02/:2 11:11a $
$Revision: 31 $
$Log: /peg/include/pconfig.hpp $
// 
// 31    10/02/:2 11:11a Ken
// 
// 30    10/02/:2 10:52a Jarret
// 
// 29    9/16/:2 10:20a Ken
// Removed "PEG_DOS" definition, re-defined as more accurate
// "PEG_STANDALONE".
// 
// 28    9/04/:2 4:01p Jim
// Removed the precaution of using FAST_BLIT with PRESS.
// 
// 27    9/04/:2 4:00p Ken
// 
// 26    9/04/:2 2:39p Jarret
// 
// 25    9/04/:2 2:27p Ken
// 
// 24    8/07/:2 2:45p Jim
// Added the S1D13A04 Epson header to the available screen drivers
// 
// 23    8/07/:2 2:32p Jim
// Updated for Solaris build
