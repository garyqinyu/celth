...
$(ODIR)/%.o : %.cpp
	@echo "[Compile... $(notdir $<) (pegapi)]"
	${Q_}$(CXX) ${CDEP_FLAG} ${CPPFLAGS_STATIC} $(CPPFLAGS) -c $< -o $@

# Implicit C++ Compile Rule
$(ODIR)/%.so : %.cpp
	@echo "[Compile... $(notdir $<) (pegapi)]"
	${Q_}$(CXX) ${CDEP_FLAG} ${CPPFLAGS_SHARED} $(CPPFLAGS) -c $< -o $@
...
#${Q_}$(CC) ${LDFLAGS}  -shared -fPIC -Wl,-map,${SH_MAP} -Wl,--cref  -o  $@ -Wl,-soname,lib${LIB}.so ${SH_OBJS} ${LIBS}
	${Q_}$(CXX) ${LDFLAGS}  -shared   -Wl,--cref  -o $@ -Wl,-soname,lib${LIB}.so ${SH_OBJS} ${LIBS}
